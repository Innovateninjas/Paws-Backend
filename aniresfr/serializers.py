...
def increment_user_reports(animal):
    try:
        user = CustomUser.objects.get(user__email=animal.user_email)
        update_fields = {
            'no_reports': F('no_reports') + 1,
            'coins': F('coins') + 2,
        }
        if (user.no_reports+1) % 5 == 0:
            update_fields['level'] = F('level') + 1
        CustomUser.objects.filter(user__email=animal.user_email).update(**update_fields)
